...
所有路由 reference 已随本技能提供。执行前先读取 `references/00-scope-and-routing.md`，再读取任务对应 reference；不得跳过入口确认直接执行脚本。

## 全局规则

- 不使用含义不清的 `amount` 直接计算金额逾期率；金额字段必须拆成明确分子和分母。
- 不默认把 `sample_id` 当作唯一分析单位；样本 ID、客户 ID、申请 ID、借据 ID、支用 ID 和分析颗粒度必须由用户确认。
- 常见字段名、常见颗粒度和常见聚合规则只能作为候选提示；读取 input 后可以生成候选映射，但必须经用户确认后才能执行分析。
...
- 每个命令均要求 `--config`、`--confirmation`、`--input`、`--output-dir`；先执行 `scripts/validate_contract.py`。
