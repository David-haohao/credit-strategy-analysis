...
3. 对准入特征用 `toad` 进行确认分箱与 WOE/IV 计算。IV 用于筛选和描述，不单独证明规则有效；输出各箱好坏分布、坏样本率和 IV 分量。按固定分箱顺序计算每箱 `bad_rate = bad_count / (good_count + bad_count)`，并在特征汇总中记录单调性：有效分箱坏样本率非降且至少一次上升为 `increasing`，非升且至少一次下降为 `decreasing`，全部相等为 `flat`，方向反复为 `non_monotonic`，有效箱不足两个为 `not_evaluated`。缺失箱独立输出，但不参与方向判定。
4. 数值特征从确认业务阈值、确认分位点的两端极端区间、chi 分箱边界和缺失值生成有限候选。当前 CLI 的最小实现仅使用 `rule_mining.numeric_cut_sources` 中确认的 `quantile:x` 与 `value:x`，生成 `feature >= cut`、`feature < cut` 和缺失规则；不得遍历所有唯一值。
...
阶段产物中的规则表达式只支持固定安全子集：比较（如 `feature >= 1000`、`feature < 3`、`feature == 'A'`）、区间（`feature between 1000 and 1500`）、缺失（`feature is missing`、`feature is not missing`）和 AND 组合（包括 `feature between 1000 and 1500 and age < 30`）。不支持 OR、括号、函数调用或任意 Python 表达式。
...
| 文件 | 最少字段 |
|---|---|
| `feature_screening.csv` | 特征、保留/排除状态、筛选原因、IV、覆盖量、`monotonicity`、稳定性状态 |
| `bin_iv_detail.csv` | 特征、`bin_order`、箱/类别、箱边界、好/坏样本、`bad_rate`、WOE、IV 分量、警告 |
...
- 完整 IV、WOE 和分箱结果来自 `toad`；若仅执行最小数值阈值候选生成，`bin_iv_detail.csv` 必须标记 `not_available`，不得伪造 IV。
