...
上述颗粒度列表只是供用户选择和补充的候选项。不得根据产品形态、字段名或行业习惯自行决定分析颗粒度。

如果用户要求表现指标，但成熟样本定义缺失，停止并询问。

如果分析颗粒度缺失，停止并询问。不得默认使用 `sample_id` 作为样本单位。

循环贷常见情况只用于提示需要追问的风险点，不得作为默认处理规则：

- 0/1 标签可能在借据级或支用级生成。
- 同一客户可能有多个借据或多次支用。
- 贷前特征通常是客户级。
- 若需要输出人维度指标，必须请用户决定从借据/支用级到客户级的聚合规则。可给出候选项，例如 `max(Y_label)`、任一借据逾期即客户逾期、最近一笔借据、首笔借据或按观察窗聚合，但不得自行选择。

### Step 3.1：字段映射确认

读取 input 后，可以根据列名、类型和取值生成字段映射候选，例如客户 ID、借据 ID、日期、标签、分数、金额和拒绝码字段。候选映射只能用于提问，不得直接执行。

必须让用户确认：

- 哪些输入列分别对应统一契约字段。
- 是否存在一列多义或多列同义。
- 必需字段缺失时，是补充字段、调整任务，还是停止分析。

统一契约中的字段名是语义标准，不要求原始数据必须提前改成这些字段名。最终确认后的映射必须写入 manifest。
